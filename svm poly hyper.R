...
#svm_model <- train(  Attrition ~ ., data = train_data, method = "svmPoly", trControl = train_control, tuneGrid = tuning_grid)
svm_model <- train(  Attrition ~ ., data = train_data[, c(selected_features, "Attrition")], method = "svmPoly", trControl = train_control, tuneGrid = tuning_grid)
